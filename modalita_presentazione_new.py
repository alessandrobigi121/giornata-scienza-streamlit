...
sezione = st.sidebar.radio(
    ...
    ["Battimenti", "Pacchetti d'Onda", "Spettro di Fourier", 
     ...
     "📥 Centro Download", "🚀 Modalità Presentazione OLD", "🚀 Modalità Presentazione NEW"]
)
...
if sezione == "🚀 Modalità Presentazione OLD":
    # ========== TITOLO PRINCIPALE ==========
    st.markdown("""
    <div style="
    ...
    ">
        ...
        ">⚛️ Il Mistero dell'Elettrone</h1>
        <p style="
        ...
        <div style="margin-top: 1.5rem; display: flex; justify-content: center; gap: 2rem;">
            <span style="color: rgba(255,255,255,0.7); font-size: 0.95rem;">👨‍🔬 Alessandro Bigi & Giovanni De Zan</span>
    ...
    if 'pres_step' not in st.session_state:
        st.session_state.pres_step = 0  # 0=vuoto, 1=onda pura, 2=due identiche, 3=battimenti
    if 'pres_n_waves' not in st.session_state:
        st.session_state.pres_n_waves = 1
    if 'pres_show_prob' not in st.session_state:
        st.session_state.pres_show_prob = False
    if 'pres_collapsed' not in st.session_state:
    ...
    styled_header(
        "🎵", 
        "Parte 1: I Battimenti",
        "Dall'onda pura ai battimenti: il suono come analogia per la particella",
        "#3498db"
    )
    
    # --- CONTROLLI STEP ---
    st.markdown("#### 🎛️ Controlli Presentazione")
    ctrl_cols = st.columns(4)
    with ctrl_cols[0]:
        if st.button("🔇 Grafico Vuoto", key="pres_s0", use_container_width=True):
            st.session_state.pres_step = 0
    with ctrl_cols[1]:
        if st.button("🎵 1 Diapason", key="pres_s1", use_container_width=True):
            st.session_state.pres_step = 1
    with ctrl_cols[2]:
        if st.button("🎵🎵 2 Identici", key="pres_s2", use_container_width=True):
            st.session_state.pres_step = 2
    with ctrl_cols[3]:
        if st.button("〰️ Battimenti (Pesetto)", key="pres_s3", use_container_width=True):
            st.session_state.pres_step = 3
    ...
    if step == 0:
        # Grafico vuoto
        fig_beat = go.Figure()
        fig_beat.add_trace(go.Scatter(x=t_pres, y=np.zeros_like(t_pres), 
                                      line=dict(color='rgba(100,100,100,0.3)', width=1), name="—"))
        fig_beat.update_layout(height=400, xaxis_title="Tempo (s)", yaxis_title="Ampiezza",
                               yaxis=dict(range=[-2.5, 2.5]))
        applica_stile(fig_beat, is_light_mode)
        st.plotly_chart(fig_beat, use_container_width=True, config=get_download_config("pres_vuoto"))
        st.info("🔇 Premi **1 Diapason** per iniziare.")
        ...
        fig_wave.add_trace(go.Bar(x=[440], y=[1.0], marker_color='#3498db', width=3, 
                                  name="f₁ = 440 Hz", showlegend=False), row=2, col=1)
        fig_wave.update_xaxes(title_text="Tempo (s)", row=1, col=1)
        fig_wave.update_xaxes(title_text="Frequenza (Hz)", range=[400, 500], row=2, col=1)
        fig_wave.update_yaxes(title_text="Ampiezza", range=[-2.5, 2.5], row=1, col=1)
        fig_wave.update_yaxes(title_text="Ampiezza", range=[0, 1.3], row=2, col=1)
        fig_wave.update_layout(height=600, showlegend=True, hovermode='x unified')
        ...
        y1 = np.cos(2 * np.pi * f1_pres * t_pres)
        y2 = np.cos(2 * np.pi * f2_same * t_pres)
        y_sum = y1 + y2  # = 2*cos(...)
        ...
        fig_same.add_trace(go.Bar(x=[440], y=[2.0], marker_color='#8e44ad', width=3, 
                                   name="f = 440 Hz", showlegend=False), row=2, col=1)
        fig_same.update_xaxes(title_text="Tempo (s)", row=1, col=1)
        fig_same.update_xaxes(title_text="Frequenza (Hz)", range=[400, 500], row=2, col=1)
        fig_same.update_yaxes(title_text="Ampiezza", range=[-2.5, 2.5], row=1, col=1)
        fig_same.update_yaxes(title_text="Ampiezza", range=[0, 2.5], row=2, col=1)
        fig_same.update_layout(height=600, showlegend=True, hovermode='x unified')
        ...
        fig_beats.add_trace(go.Bar(x=[440, 445], y=[1.0, 1.0], 
                                    marker_color=['#3498db', '#e74c3c'], width=2,
                                    name="Componenti", showlegend=False), row=2, col=1)
        fig_beats.update_xaxes(title_text="Tempo (s)", row=1, col=1)
        fig_beats.update_xaxes(title_text="Frequenza (Hz)", range=[400, 500], row=2, col=1)
        fig_beats.update_yaxes(title_text="Ampiezza", range=[-2.5, 2.5], row=1, col=1)
        fig_beats.update_yaxes(title_text="Ampiezza", range=[0, 1.3], row=2, col=1)
        fig_beats.update_layout(height=600, showlegend=True, hovermode='x unified')
        ...
        dur_audio = st.slider("Durata audio (s)", 1.0, 5.0, 3.0, 0.5, key="pres_dur_audio_beat")
        if st.button("▶️ Riproduci", key="pres_play_beat"):
            t_audio = np.linspace(0, dur_audio, int(SAMPLE_RATE * dur_audio))
            y_audio = np.sin(2*np.pi*f1_pres*t_audio) + np.sin(2*np.pi*f2_beat*t_audio)
            st.audio(genera_audio(y_audio), format='audio/wav')
    # ========== SLIDE 2: PACCHETTO D'ONDA ==========
    st.markdown("---")
    styled_header(
        "🌊", 
        "Parte 2: Il Pacchetto d'Onda e la Probabilità",
        "Costruiamo un pacchetto d'onda sommando molte frequenze",
        "#9b59b6"
    )
    
    st.markdown("#### 🎛️ Costruzione del Pacchetto")
    
    # Controlli Onde
    wave_cols = st.columns([1, 1, 1, 1, 2])
    with wave_cols[0]:
        if st.button("➕ Aggiungi Onda", key="pres_add_wave", use_container_width=True):
            st.session_state.pres_n_waves = min(st.session_state.pres_n_waves + 1, 200)
    with wave_cols[1]:
        if st.button("➖ Rimuovi Onda", key="pres_rem_wave", use_container_width=True):
            st.session_state.pres_n_waves = max(st.session_state.pres_n_waves - 1, 1)
    with wave_cols[2]:
        if st.button("🔟 N = 10", key="pres_n10", use_container_width=True):
            st.session_state.pres_n_waves = 10
    with wave_cols[3]:
        if st.button("5️⃣0️⃣ N = 50", key="pres_n50", use_container_width=True):
            st.session_state.pres_n_waves = 50
    with wave_cols[4]:
        st.markdown(f"### Onde: **N = {st.session_state.pres_n_waves}**")
    
    n_w = st.session_state.pres_n_waves
    f_min_pk = 100.0
    f_max_pk = 200.0
    durata_pk = 0.15
    t_pk = np.linspace(-durata_pk, durata_pk, int(durata_pk * 2 * 20000))
    freqs_pk = np.linspace(f_min_pk, f_max_pk, n_w)
    
    # Calcolo pacchetto
    y_packet = np.zeros_like(t_pk)
    y_components = []
    for f in freqs_pk:
        comp = (1.0 / n_w) * np.cos(2 * np.pi * f * t_pk)
        y_components.append(comp)
        y_packet += comp
    
    # Inviluppo
    pad_l = int(len(t_pk) * 0.1)
    y_pad_pk = np.pad(y_packet, (pad_l, pad_l), mode='reflect')
    env_pk = np.abs(signal.hilbert(y_pad_pk))[pad_l:-pad_l]
    
    mostra_comp = st.checkbox("🌈 Mostra onde componenti", False, key="pres_show_comp_pk")
    
    fig_pkt = make_subplots(rows=1, cols=1, subplot_titles=(f"Pacchetto d'Onda: N = {n_w} onde ({f_min_pk:.0f} - {f_max_pk:.0f} Hz)",))
    
    if mostra_comp and n_w <= 50:

        for i, comp in enumerate(y_components):
            hue = i / max(n_w, 1)
            r, g, b = colorsys.hsv_to_rgb(hue, 0.8, 0.9)
            color_str = f"rgb({int(r*255)},{int(g*255)},{int(b*255)})"
            fig_pkt.add_trace(go.Scatter(x=t_pk*1000, y=comp, line=dict(color=color_str, width=0.5),
                                         opacity=0.35, showlegend=False))
    
    fig_pkt.add_trace(go.Scatter(x=t_pk*1000, y=y_packet, line=dict(color='#2c3e50', width=2.5), 
                                  name="Pacchetto Σ"))
    fig_pkt.add_trace(go.Scatter(x=t_pk*1000, y=env_pk, line=dict(color='#e74c3c', width=2, dash='dash'), 
                                  name="Inviluppo"))
    fig_pkt.add_trace(go.Scatter(x=t_pk*1000, y=-env_pk, showlegend=False,
                                  line=dict(color='#e74c3c', width=2, dash='dash')))
    
    fig_pkt.update_xaxes(title_text="Tempo (ms)", range=[-150, 150])
    fig_pkt.update_yaxes(title_text="Ampiezza", range=[-1.2, 1.2])
    fig_pkt.update_layout(height=500, hovermode='x unified')
    ...
    for i, f in enumerate(freqs_anim):
        ...
        r, g, b = colorsys.hsv_to_rgb(hue, 0.8, 0.9)
        color_str = f"rgb({int(r*255)},{int(g*255)},{int(b*255)})"
        fig_race.add_trace(go.Scatter(x=t_anim*1000, y=y_i, line=dict(color=color_str, width=1),
                                       opacity=0.5, name=f"f={f:.0f} Hz"))
    ...
    **Louis de Broglie** (1924) legò la frequenza di un elettrone alla sua quantità di moto:
    ...
    styled_header(
        "🎲", 
        "Parte 3: Probabilità e Misura",
        "La densità di probabilità e il collasso della funzione d'onda",
        "#27ae60"
    )
    ...
    with prob_cols[0]:
        if st.button("📊 Mostra Probabilità |Ψ|²", key="pres_toggle_prob", use_container_width=True):
            st.session_state.pres_show_prob = not st.session_state.pres_show_prob
            st.session_state.pres_collapsed = False
    with prob_cols[1]:
    ...
    <div style="
        ...
        padding: 2.5rem; border-radius: 20px; text-align: center; margin-top: 2rem;
    ">
        <h2 style="color: white; margin: 0 0 1rem 0;">🎓 Principio di Indeterminazione di Heisenberg</h2>
        <div style="
        ...
        <p style="color: rgba(255,255,255,0.85); margin-top: 1.5rem; font-size: 1.1rem; font-style: italic;">
            «Dobbiamo ricordare che ciò che osserviamo non è la natura in sé,<br>
            ma la natura esposta al nostro modo di porre le domande.»
        </p>
        <p style="color: rgba(255,255,255,0.6); margin-top: 0.5rem;">— Werner Heisenberg</p>
...
# ========== MODALITÀ PRESENTAZIONE NEW ==========
if sezione == "🚀 Modalità Presentazione NEW":
    # ========== TITOLO PRINCIPALE ==========
    st.markdown("""
    <div style="
        background: linear-gradient(135deg, #1a1a2e 0%, #16213e 50%, #0f3460 100%);
    ...
    # ========== SESSION STATE INIT ==========
    if 'pnew_step' not in st.session_state:
        st.session_state.pnew_step = 0
    if 'pnew_n_waves' not in st.session_state:
        st.session_state.pnew_n_waves = 1
    if 'pnew_show_prob' not in st.session_state:
        st.session_state.pnew_show_prob = False
    if 'pnew_collapsed' not in st.session_state:
        st.session_state.pnew_collapsed = False
    if 'pnew_collapse_x' not in st.session_state:
        st.session_state.pnew_collapse_x = 0.0

    # ================================================================
    # PARTE 1: INTRODUZIONE — La doppia natura dell'elettrone
    # ================================================================
    st.markdown("---")
    styled_header(
        "🔬",
        "Introduzione: L'Oggetto Quantistico",
        "L'elettrone non è né solo onda né solo particella — è un oggetto quantistico",
        "#e74c3c"
    )

    st.markdown("""
    <div style="
        background: linear-gradient(145deg, rgba(231,76,60,0.08), rgba(231,76,60,0.02));
        border-left: 4px solid #e74c3c;
        padding: 1.5rem 2rem; border-radius: 0 12px 12px 0; margin: 1rem 0;
    ">
        <p style="font-size: 1.05rem; line-height: 1.7;">
        L'elettrone ha una <strong>doppia natura</strong>: in certi esperimenti si comporta come una particella,
        in altri come un'onda. La risposta a questa dualità è <strong>matematica</strong>.
        </p>
    </div>
    """, unsafe_allow_html=True)

    # --- Funzione d'onda armonica ---
    st.markdown("##### 📐 Funzione d'onda armonica")
    st.markdown("Un'onda armonica sonora è descritta dalla funzione:")
    st.latex(r"y(x,t) = A\cos(\omega t - kx)")
    st.markdown("In un punto fisso $x=0$:")
    st.latex(r"y(0,t) = A\cos(\omega t)")
    ...
    ">
        ...
        Lunghezza d'onda: λ = v/f &nbsp;|&nbsp;
        Numero d'onda: k = 2π/λ = 2π·f/v
    </div>
    """, unsafe_allow_html=True)

    # --- Funzione d'onda di Schrödinger ---
    st.markdown("##### ⚛️ La Funzione d'Onda ψ(x,t)")
    st.markdown("""
    **Schrödinger** (1926) propose una funzione d'onda **ψ(x,t)** per descrivere il moto dell'elettrone.
    Essa contiene **tutte le informazioni** sulla particella.
    """)



    # ================================================================
    # PARTE 2: I BATTIMENTI
    # ================================================================
    st.markdown("---")
    styled_header(
        "🎵",
    ...
    ctrl_cols = st.columns(4)
    with ctrl_cols[0]:
        if st.button("🔇 Grafico Vuoto", key="pnew_s0", use_container_width=True):
            st.session_state.pnew_step = 0
    with ctrl_cols[1]:
        if st.button("🎵 1 Diapason", key="pnew_s1", use_container_width=True):
            st.session_state.pnew_step = 1
    with ctrl_cols[2]:
        if st.button("🎵🎵 2 Identici", key="pnew_s2", use_container_width=True):
            st.session_state.pnew_step = 2
    with ctrl_cols[3]:
        if st.button("〰️ Battimenti (Pesetto)", key="pnew_s3", use_container_width=True):
            st.session_state.pnew_step = 3

    step = st.session_state.pnew_step
    ...
    if step == 0:
        ...
        fig_beat.update_layout(height=400, xaxis_title="Tempo (s)", yaxis_title="Ampiezza",
                               yaxis=dict(range=[-2.5, 2.5]))
        applica_stile(fig_beat, is_light_mode)
        st.plotly_chart(fig_beat, use_container_width=True, config=get_download_config("pnew_vuoto"))
        st.info("🔇 Premi **1 Diapason** per iniziare.")
        ...
        fig_wave = make_subplots(rows=2, cols=1,
                                 subplot_titles=("Onda Pura: 440 Hz", "Spettro di Frequenze (Spazio delle Fasi)"),
                                 vertical_spacing=0.25, row_heights=[0.65, 0.35])
        fig_wave.add_trace(go.Scatter(x=t_pres, y=y1, line=dict(color='#3498db', width=2),
                                      name="Onda 1 (440 Hz)"), row=1, col=1)
        fig_wave.add_trace(go.Bar(x=[440], y=[1.0], marker_color='#3498db', width=3,
        ...
        fig_wave.update_xaxes(title_text="Frequenza (Hz)", range=[400, 500], row=2, col=1)
        fig_wave.update_yaxes(title_text="Ampiezza", range=[-2.5, 2.5], row=1, col=1)
        fig_wave.update_yaxes(title_text="Intensità", range=[0, 1.3], row=2, col=1)
        fig_wave.update_layout(height=600, showlegend=True, hovermode='x unified')
        applica_stile(fig_wave, is_light_mode)
        st.plotly_chart(fig_wave, use_container_width=True, config=get_download_config("pnew_onda_pura"))

        # Info box: come leggere i due grafici
        st.markdown("""
        <div style="
            border-left: 4px solid #3498db;
            padding: 1rem 1.5rem; border-radius: 0 10px 10px 0; margin: 0.5rem 0;
            background: rgba(52,152,219,0.05);
        ">
            <strong>📊 Come leggere:</strong><br>
            • <strong>Sopra</strong>: grafico di y (ampiezza) in funzione del tempo t<br>
            • <strong>Sotto</strong>: grafico dell'intensità in funzione delle frequenze (spazio delle fasi) — un solo picco netto
        </div>
        """, unsafe_allow_html=True)
        ...
        fig_same.update_xaxes(title_text="Frequenza (Hz)", range=[400, 500], row=2, col=1)
        fig_same.update_yaxes(title_text="Ampiezza", range=[-2.5, 2.5], row=1, col=1)
        fig_same.update_yaxes(title_text="Intensità", range=[0, 2.5], row=2, col=1)
        fig_same.update_layout(height=600, showlegend=True, hovermode='x unified')
        applica_stile(fig_same, is_light_mode)
        st.plotly_chart(fig_same, use_container_width=True, config=get_download_config("pnew_due_identiche"))

        st.info("Interferenza costruttiva: ampiezza maggiore, ma l'onda resta identica nella struttura. Non risolve il problema della localizzazione.")
        ...
        fig_beats.update_xaxes(title_text="Frequenza (Hz)", range=[400, 500], row=2, col=1)
        fig_beats.update_yaxes(title_text="Ampiezza", range=[-2.5, 2.5], row=1, col=1)
        fig_beats.update_yaxes(title_text="Intensità", range=[0, 1.3], row=2, col=1)
        fig_beats.update_layout(height=600, showlegend=True, hovermode='x unified')
        applica_stile(fig_beats, is_light_mode)
        st.plotly_chart(fig_beats, use_container_width=True, config=get_download_config("pnew_battimenti"))

        # --- FORMULE PROSTAFERESI E INTENSITÀ ---
        st.markdown("#### 📐 Formula dei Battimenti e Intensità")
        st.markdown("Per il **Principio di Sovrapposizione**, la somma di due coseni:")
        st.latex(r"y(t) = A \cos(\omega_1 t) + A \cos(\omega_2 t)")
        st.markdown("diventa, grazie alle **formule di prostaferesi**, un prodotto:")
        st.latex(r"y(t) = \underbrace{2A \cos\left(\frac{\omega_1 - \omega_2}{2}t\right)}_{\text{Inviluppo (Modulazione)}} \cdot \underbrace{\cos\left(\frac{\omega_1 + \omega_2}{2}t\right)}_{\text{Portante (Nota)}}")

        st.markdown("L'orecchio (o un rilevatore) percepisce l'**Intensità**, proporzionale al quadrato dell'ampiezza:")
        st.latex(r"I(t) \propto y^2(t) \propto \cos^2\left(\frac{\Delta\omega}{2}t\right)")

        st.markdown("Di conseguenza, la frequenza del battimento è $\Delta f$ e il suo **periodo** è:")
        st.latex(r"T_{batt} = \frac{1}{\Delta f}")
        ...
        <div style="
            ...
            background: linear-gradient(145deg, rgba(255,255,255,0.03), rgba(128,128,128,0.05));
        ">
            <strong>1.</strong> La <strong>Portante</strong> è l'oscillazione veloce — la nota percepita.<br>
            <strong>2.</strong> L'<strong>Inviluppo</strong> è l'oscillazione lenta — la "forma" del suono (massimi e minimi).<br><br>
            ⚠️ <strong>Il seme dell'indeterminazione:</strong> Notare che T<sub>batt</sub> = 1/Δf. Se riduciamo lo scarto di frequenza (Δf piccolo), il tempo tra due battimenti si dilata (T grande).
        </div>
        """, unsafe_allow_html=True)

        # Audio
        st.markdown("#### 🔊 Ascolta i Battimenti")
        dur_audio = st.slider("Durata audio (s)", 1.0, 5.0, 3.0, 0.5, key="pnew_dur_audio_beat")
        if st.button("▶️ Riproduci", key="pnew_play_beat"):
            t_audio = np.linspace(0, dur_audio, int(SAMPLE_RATE * dur_audio))
            y_audio = np.sin(2*np.pi*f1_pres*t_audio) + np.sin(2*np.pi*f2_beat*t_audio)
            st.audio(genera_audio(y_audio), format='audio/wav')

    # ================================================================
    # PARTE 3: IL PACCHETTO D'ONDA E LA PROBABILITÀ
    # ================================================================
    st.markdown("---")
    styled_header(
        "🌊",
    ...
    st.markdown("#### 📐 L'Integrale di Fourier")
    st.markdown("Passando da 2 onde a infinite onde, la sommatoria diventa un **integrale di Fourier**:")
    st.latex(r"\Psi(x,t) = \int_{-\infty}^{+\infty} \phi(k) e^{i(kx - \omega t)} dk")
    st.markdown("Costruiamo ora noi un pacchetto sommando un grande numero di onde:")

    st.markdown("#### 🎛️ Costruzione del Pacchetto")

    wave_cols = st.columns([1, 1, 1, 1, 2])
    with wave_cols[0]:
        if st.button("➕ Aggiungi Onda", key="pnew_add_wave", use_container_width=True):
            st.session_state.pnew_n_waves = min(st.session_state.pnew_n_waves + 1, 200)
    with wave_cols[1]:
        if st.button("➖ Rimuovi Onda", key="pnew_rem_wave", use_container_width=True):
            st.session_state.pnew_n_waves = max(st.session_state.pnew_n_waves - 1, 1)
    with wave_cols[2]:
        if st.button("🔟 N = 10", key="pnew_n10", use_container_width=True):
            st.session_state.pnew_n_waves = 10
    with wave_cols[3]:
        if st.button("5️⃣0️⃣ N = 50", key="pnew_n50", use_container_width=True):
            st.session_state.pnew_n_waves = 50
    with wave_cols[4]:
        st.markdown(f"### Onde: **N = {st.session_state.pnew_n_waves}**")

    n_w = st.session_state.pnew_n_waves
    f_min_pk = 100.0
    f_max_pk = 200.0
    durata_pk = 0.15
    t_pk = np.linspace(-durata_pk, durata_pk, int(durata_pk * 2 * 20000))
    ...
    mostra_comp = st.checkbox("🌈 Mostra onde componenti", False, key="pnew_show_comp_pk")
    ...
    fig_pkt.update_layout(height=500, hovermode='x unified')
    applica_stile(fig_pkt, is_light_mode)
    st.plotly_chart(fig_pkt, use_container_width=True, config=get_download_config("pnew_pacchetto"))

    # Spiegazione interferenza
    st.markdown("""
    <div style="
    ...
    ">
        <strong>Perché il pacchetto si forma?</strong><br>
        • Al <strong>centro</strong>: tutte le sinusoidi sono perfettamente in fase → la somma è altissima.<br>
        • <strong>Allontanandosi</strong>: le onde iniziano a sfasarsi → interferenza distruttiva.<br>
        • Come in una gara di corsa: alla partenza tutti sono allineati, ma poi ognuno va a una velocità diversa e il gruppo si sfalda.
    </div>
    """, unsafe_allow_html=True)
    ...
    if 'pnew_race_playing' not in st.session_state:
        st.session_state.pnew_race_playing = False
    if 'pnew_race_phase' not in st.session_state:
        st.session_state.pnew_race_phase = 0.0

    anim_cols = st.columns(3)
    with anim_cols[0]:
        if st.button("▶️ Play", key="pnew_race_play", use_container_width=True):
            st.session_state.pnew_race_playing = True
            st.session_state.pnew_race_phase = 0.0
    with anim_cols[1]:
        if st.button("⏸️ Pausa", key="pnew_race_pause", use_container_width=True):
            st.session_state.pnew_race_playing = False
    with anim_cols[2]:
        if st.button("🔄 Reset", key="pnew_race_reset", use_container_width=True):
            st.session_state.pnew_race_playing = False
            st.session_state.pnew_race_phase = 0.0

    phase_shift = st.session_state.pnew_race_phase

    n_anim = 10
    freqs_anim = np.linspace(100, 400, n_anim)
    t_anim = np.linspace(-0.002, 0.002, 2000)
    ...
    fig_race.update_layout(height=450, hovermode='x unified',
                           legend=dict(font=dict(size=9)))
    applica_stile(fig_race, is_light_mode)
    st.plotly_chart(fig_race, use_container_width=True, config=get_download_config("pnew_gara"))
    ...
    applica_stile(fig_stick, is_light_mode)
    st.plotly_chart(fig_stick, use_container_width=True, config=get_download_config("pnew_stickman"))

    if st.session_state.pnew_race_playing:
        import time
        time.sleep(0.08)
        st.session_state.pnew_race_phase += 0.02
        if st.session_state.pnew_race_phase > 1.0:
            st.session_state.pnew_race_playing = False
        else:
            st.rerun()

    # --- DENSITÀ DI PROBABILITÀ |Ψ|² ---
    st.markdown("---")
    st.markdown("#### 📊 Densità di Probabilità |Ψ|²")
    ...
    y_pad_prob = np.pad(y_prob, (pad_prob, pad_prob), mode='reflect')
    env_prob = np.abs(signal.hilbert(y_pad_prob))[pad_prob:-pad_prob]
    t_prob_ms = t_prob * 1000

    prob_cols = st.columns(2)
    with prob_cols[0]:
        if st.button("📊 Mostra |Ψ|²", key="pnew_toggle_prob", use_container_width=True):
            st.session_state.pnew_show_prob = not st.session_state.pnew_show_prob
            st.session_state.pnew_collapsed = False
    with prob_cols[1]:
        if st.button("⚡ MISURA", key="pnew_misura", use_container_width=True):
            prob_dist = env_prob**2
            prob_dist = prob_dist / (np.sum(prob_dist) + 1e-10)
            idx_collapse = np.random.choice(len(t_prob), p=prob_dist)
            st.session_state.pnew_collapse_x = t_prob_ms[idx_collapse]
            st.session_state.pnew_show_prob = True
            st.session_state.pnew_collapsed = True

    if st.session_state.pnew_collapsed:
        collapse_x = st.session_state.pnew_collapse_x
        t_ms = t_prob_ms
        ...
        for i in range(n_frames):
            ...
            prob_c = prob_c / (np.max(prob_c) + 1e-10) if np.max(prob_c) > 0 else prob_c
            frame_data = [
                go.Scatter(x=t_ms, y=y_prob, line=dict(color='rgba(100,100,100,0.15)', width=1)),
                go.Scatter(x=t_ms, y=y_c, line=dict(color='#8e44ad', width=2)),
                go.Scatter(x=t_ms, y=prob_c, fill='tozeroy',
                           fillcolor='rgba(231, 76, 60, 0.3)',
                           line=dict(color='#e74c3c', width=2)),
            ]
            frames.append(go.Frame(data=frame_data, name=str(i)))
        ...
        fig_collapse.update_layout(
            ...
            updatemenus=[dict(
                ...
                buttons=[
                    dict(label="▶ Collasso!", method="animate",
        ...
        applica_stile(fig_collapse, is_light_mode)
        st.plotly_chart(fig_collapse, use_container_width=True, config=get_download_config("pnew_collasso"))
        st.caption("Premi **▶ Collasso!** per vedere la funzione d'onda collassare nel punto di misura.")

    elif st.session_state.pnew_show_prob:
        fig_prob = go.Figure()
        fig_prob.add_trace(go.Scatter(x=t_prob_ms, y=y_prob,
                                      line=dict(color='#2c3e50', width=2), name="Ψ(x)"))
        prob_curve = env_prob**2
        ...
        fig_prob.update_layout(height=450, xaxis_title="Posizione (ms)", yaxis_title="Ampiezza",
                               yaxis=dict(range=[-1.2, 1.5]))
        applica_stile(fig_prob, is_light_mode)
        st.plotly_chart(fig_prob, use_container_width=True, config=get_download_config("pnew_probabilita"))
        ...
        ">
            <strong>Interpretazione di Born:</strong> ψ² esprime la probabilità di trovare la particella
            in un intervallo Δx in un dato istante t. Dove la curva è alta, è molto probabile trovare l'elettrone.
        </div>
        """, unsafe_allow_html=True)
        ...
        fig_prob.update_layout(height=450, xaxis_title="Posizione (ms)", yaxis_title="Ampiezza",
                               yaxis=dict(range=[-1.2, 1.5]))
        applica_stile(fig_prob, is_light_mode)
        st.plotly_chart(fig_prob, use_container_width=True, config=get_download_config("pnew_pacchetto_prob"))
    ...
    # --- Formula Δk = (2π/v)·Δf ---
    st.markdown("#### 📐 Relazione tra Δk e Δf")
    st.latex(r"k = \frac{2\pi}{\lambda} = 2\pi \cdot \frac{f}{v}")
    st.latex(r"\Rightarrow \quad \boxed{\Delta k = \frac{2\pi}{v} \cdot \Delta f}")

    scenario_pres = st.radio(
        "Seleziona scenario:",
        ["Pacchetto Standard", "Super-Localizzato (Δk grande)", "Quasi-Monocromatico (Δk piccolo)"],
        key="pnew_scenario_ind",
        horizontal=True
    )
    ...
    with col_ind1:
        ...
        fig_space.update_layout(height=400, xaxis_title="Posizione x (m)", yaxis_title="A(x)",
                                yaxis=dict(range=[-1.2, 1.2]))
        applica_stile(fig_space, is_light_mode)
        st.plotly_chart(fig_space, use_container_width=True, config=get_download_config("pnew_spazio"))
    ...
    with col_ind2:
        ...
        fig_freq.update_layout(height=400, xaxis_title="Frequenza (Hz)", yaxis_title="Ampiezza",
                               ...
                               xaxis=dict(range=[0, 250]), yaxis=dict(range=[0, 0.05]))
        applica_stile(fig_freq, is_light_mode)
        st.plotly_chart(fig_freq, use_container_width=True, config=get_download_config("pnew_spettro"))

    # Spiegazione chiave della prof
    st.markdown("""
    <div style="
        background: linear-gradient(145deg, rgba(243,156,18,0.1), rgba(243,156,18,0.02));
        border-left: 4px solid #f39c12;
        padding: 1.5rem 2rem; border-radius: 0 12px 12px 0; margin: 1rem 0;
    ">
        <p style="font-size: 1.05rem; line-height: 1.7;">
        Per avere l'onda "localizzata" in un intervallo <strong>Δx</strong>, ho dovuto sommare <strong>N onde
        monocromatiche</strong> con frequenze comprese nell'intervallo <strong>Δf</strong> (equivalentemente Δk).<br><br>
        Se l'onda è localizzata (pacchetto stretto), <strong>non posso dire con precisione che frequenza abbia</strong>,
        perché lo spettro è largo. E viceversa: se lo spettro è stretto (frequenza precisa), l'onda si "spalma" ovunque.
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    ">
        ...
        <p style="color: rgba(255,255,255,0.9); margin-top: 0.5rem; font-size: 1rem;">
            Non è un limite dei nostri strumenti — è una proprietà intrinseca della natura ondulatoria!
        </p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with dyn_col1:
        ...
        fig_dyn_space.update_layout(height=350, xaxis_title="t (ms)", yaxis_title="A(t)",
                                    xaxis=dict(range=[-300, 300]), yaxis=dict(range=[-1.2, 1.2]))
        applica_stile(fig_dyn_space, is_light_mode)
        st.plotly_chart(fig_dyn_space, use_container_width=True, config=get_download_config("pnew_dyn_space"))
    ...
    with dyn_col2:
        ...
        fig_dyn_freq.update_layout(height=350, xaxis_title="Frequenza (Hz)", yaxis_title="Ampiezza",
                                   ...
                                   xaxis=dict(range=[0, 350]), yaxis=dict(range=[0, 0.05]))
        applica_stile(fig_dyn_freq, is_light_mode)
        st.plotly_chart(fig_dyn_freq, use_container_width=True, config=get_download_config("pnew_dyn_freq"))

    # --- Passaggio a de Broglie ---
    st.markdown("---")
    st.markdown("#### 🔗 Da Δk a Δp: la relazione di de Broglie")
    st.markdown("Utilizzando la relazione di **Louis de Broglie** (1924) $\lambda = h/p$ e la definizione $k = 2\pi/\lambda$, possiamo scrivere:")
    st.latex(r"p = \hbar k \quad \text{con } \hbar = \frac{h}{2\pi}")
    st.markdown("Derivando, otteniamo:")
    st.latex(r"\boxed{\Delta p = \hbar \Delta k}")
    st.markdown("""
    L'incertezza sulla frequenza ($\\Delta k$) si trasforma direttamente
    in incertezza sulla **quantità di moto** ($\\Delta p$) dell'elettrone.
    """)
    ...
    styled_header(
        "🎓",
        "Parte 5: Da de Broglie a Heisenberg",
        "Il principio di indeterminazione — la derivazione completa",
        "#1abc9c"
    )

    st.markdown("##### 📐 Derivazione del Principio di Indeterminazione")

    st.markdown("A partire dal Teorema di Fourier, si può dimostrare che un impulso periodico è tanto più localizzato quanto maggiore è il numero di onde elementari che servono per generarlo. Per un pacchetto gaussiano (il caso di minima incertezza), questa proprietà si esprime come:")
    st.latex(r"\Delta x \cdot \Delta k \geq \frac{1}{2}")

    st.markdown(r"Moltiplichiamo entrambi i membri per la costante di Planck ridotta $\hbar$:")
    st.latex(r"\Delta x \cdot (\hbar \Delta k) \geq \frac{\hbar}{2}")

    st.markdown(r"Sostituendo la relazione $\Delta p = \hbar \Delta k$ appena trovata:")
    st.latex(r"\boxed{\Delta x \cdot \Delta p \geq \frac{\hbar}{2}}")
    ...
    ">
        ...
        <div style="
            ...
            border-radius: 10px; display: inline-block;
        ">
            <span style="color: #f1c40f; font-size: 1.8rem; font-weight: 700;">Δx · Δp ≥ ħ/2</span>
        </div>
        <p style="color: rgba(255,255,255,0.85); margin-top: 1.5rem; font-size: 1rem;">
            Esistono coppie di grandezze coniugate tali che non è possibile misurarle
            simultaneamente con precisione arbitraria.
        </p>
        <p style="color: rgba(255,255,255,0.85); margin-top: 1rem; font-size: 1.05rem; font-style: italic;">
            «Dobbiamo ricordare che ciò che osserviamo non è la natura in sé,<br>
            ma la natura esposta al nostro modo di porre le domande.»
        </p>
        <p style="color: rgba(255,255,255,0.6); margin-top: 0.5rem;">— Werner Heisenberg</p>
    ...
    # --- Conclusione: MC vs MQ ---
    st.markdown("---")
    st.markdown("#### 🔄 Meccanica Classica vs Meccanica Quantistica")
    st.markdown("""
    <div style="display: flex; gap: 1.5rem; margin: 1rem 0;">
        ...
        ">
            <h4 style="margin: 0 0 0.5rem 0;">📏 Meccanica Classica</h4>
            <p style="font-size: 0.95rem; line-height: 1.6;">
                Visione <strong>deterministica</strong>: conosciute le forze, posizioni e velocità in un dato istante,
                è possibile conoscere la traiettoria della particella in ogni istante successivo.
                Le incertezze sono legate alle imprecisioni degli strumenti.
            </p>
        </div>
        <div style="
            flex: 1; padding: 1.5rem; border-radius: 12px;
        ...
        ">
            <h4 style="margin: 0 0 0.5rem 0;">⚛️ Meccanica Quantistica</h4>
            <p style="font-size: 0.95rem; line-height: 1.6;">
                <strong>Nega l'esistenza di traiettorie continue</strong>: la conoscenza simultanea 
                di posizione e velocità di ogni singola particella non è possibile.
                La descrizione avviene tramite ψ(x,t) e |ψ|² esprime la probabilità.
                L'indeterminazione è soggetta a regole esatte date dal Principio di Heisenberg.
            </p>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    <div style="
        ...
        border: 2px solid rgba(26,188,156,0.4); border-radius: 12px;
        background: rgba(26,188,156,0.05);
    ">
        <p style="font-size: 1.05rem; line-height: 1.7;">
            <strong>Non è un limite dei nostri strumenti.</strong> È una proprietà <strong>intrinseca, ontologica</strong>,
            della natura: in nessun senso si può ritenere che una microparticella possieda, nel senso che io possa misurare,
            in un dato istante una posizione e una velocità ben precise.
        </p>
    </div>
    """, unsafe_allow_html=True)

    st.write("")  # SPAZIATURA RICHIESTA

    # Interpretazione di Born (opzionale, come suggerito dalla prof)
    with st.expander("📖 Approfondimento: Interpretazione di Born"):
        st.markdown("""
        **Born** diede un significato fisico alla funzione d'onda ψ(x,t):
        
        **ψ²** esprime la **probabilità** di trovare la particella in un intervallo di spazio Δx
        in un dato istante t. Non possiamo sapere dove si trova la particella; possiamo solo sapere
        con quale probabilità potremmo trovarla in un intervallo Δx nel momento in cui facciamo la misura.
        """)
...
# ========== CENTRO DOWNLOAD ==========
    ...
    def dl_applica_font(fig):
        ...
        base_legend = int(11 * dl_font_scale)
        
        fig.update_layout(
            font=dict(size=base_tick),
            title_font=dict(size=base_title),
            legend=dict(font=dict(size=base_legend)),
        )
    ...
    dl_applica_font(fig_int_sim)
    st.plotly_chart(fig_int_sim, use_container_width=True, config=dl_config("pacchetto_intensita_simmetrica"))
    
    # 2e. Componenti singole (solo onde, no somma)
    st.markdown(f"#### 2e. Onde Componenti — {dl_n_onde} sinusoidi")
    fig_dl_comp = go.Figure()
    for i, f in enumerate(freq_p):
        hue = i / max(dl_n_onde, 1)
        r, g, b = colorsys.hsv_to_rgb(hue, 0.8, 0.9)
        color_str = f"rgb({int(r*255)},{int(g*255)},{int(b*255)})"
        y_c = (1/dl_n_onde) * np.cos(2 * np.pi * f * t_sim_dl)
        fig_dl_comp.add_trace(go.Scatter(x=t_sim_dl*1000, y=y_c,
                                          line=dict(color=color_str, width=max(dl_lw*0.4, 0.5)),
                                          name=f"f={f:.1f} Hz", showlegend=(i < 15)))
    fig_dl_comp.update_layout(xaxis_title="Tempo (ms)", yaxis_title="Ampiezza", height=500,
                               hovermode='x unified',
                               title=f"Onde Componenti: {dl_n_onde} sinusoidi ({dl_fmin}-{dl_fmax} Hz)")
    applica_stile(fig_dl_comp, is_light_mode)
    dl_applica_font(fig_dl_comp)
    st.plotly_chart(fig_dl_comp, use_container_width=True, config=dl_config("pacchetto_componenti"))
    
    # 2f. Gara di Corsa (Snapshot)
    st.markdown("#### 2f. Gara di Corsa (Snapshot)")
    st.caption("Mostra le N onde che si sfasano nel tempo — utile per fotografare il 'caos' distruttivo.")
    
    dl_n_race = min(dl_n_onde, 15)
    dl_phase_shift = st.slider("⏱️ Tempo (sfasamento)", 0.0, 1.0, 0.0, 0.01, key="dl_race_phase",
                                help="Sposta per scegliere l'istante da fotografare.")
    
    dl_freqs_race = np.linspace(dl_fmin, dl_fmax, dl_n_race)
    dl_t_race = np.linspace(-0.05, 0.05, 2000)
    
    fig_dl_race = go.Figure()
    dl_y_race_sum = np.zeros_like(dl_t_race)
    
    for i, f in enumerate(dl_freqs_race):
        phase = 2 * np.pi * f * dl_phase_shift * 0.01
        y_i = (1.0 / dl_n_race) * np.cos(2 * np.pi * f * dl_t_race + phase)
        dl_y_race_sum += y_i
        hue = i / max(dl_n_race, 1)
        r, g, b = colorsys.hsv_to_rgb(hue, 0.8, 0.9)
        color_str = f"rgb({int(r*255)},{int(g*255)},{int(b*255)})"
        fig_dl_race.add_trace(go.Scatter(x=dl_t_race*1000, y=y_i,
                                          line=dict(color=color_str, width=max(dl_lw*0.6, 0.8)),
                                          opacity=0.6, name=f"f={f:.0f} Hz"))
    
    fig_dl_race.add_trace(go.Scatter(x=dl_t_race*1000, y=dl_y_race_sum,
                                      line=dict(color='#2c3e50', width=dl_lw*1.2),
                                      name="SOMMA"))
    
    fig_dl_race.update_xaxes(title_text="Tempo (ms)", range=[-50, 50])
    fig_dl_race.update_yaxes(title_text="Ampiezza", range=[-1.2, 1.2])
    fig_dl_race.update_layout(height=500, hovermode='x unified',
                               legend=dict(font=dict(size=9)),
                               title=f"Gara di Corsa — sfasamento: {dl_phase_shift:.2f}")
    applica_stile(fig_dl_race, is_light_mode)
    dl_applica_font(fig_dl_race)
    st.plotly_chart(fig_dl_race, use_container_width=True, config=dl_config("gara_di_corsa_snapshot"))
